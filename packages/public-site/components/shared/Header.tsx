...
export default function Header() {
  ...
  useEffect(() => {
    ...
    document.fonts.ready.then(() => {
      ...
      const mobileBtnEl = container.querySelector('.mobile-nav-element.header-btn') as HTMLElement;
      const hamburgerEl = container.querySelector('.mobile-nav-element.hamburger') as HTMLElement;
      const savedMobile = new Map<HTMLElement, string>();
      const forceShowMobile = (el: HTMLElement | null) => {
        if (!el) return;
        savedMobile.set(el, el.getAttribute('style') || '');
        el.style.setProperty('display', 'flex', 'important');
        el.style.setProperty('visibility', 'hidden');
      };
      forceShowMobile(mobileBtnEl);
      forceShowMobile(hamburgerEl);

      const mobileBtnWidth = mobileBtnEl?.getBoundingClientRect().width || 0;
      const hamburgerWidth = hamburgerEl?.getBoundingClientRect().width || 0;

      // Restore mobile element styles
      savedMobile.forEach((style, el) => {
        if (style) el.setAttribute('style', style);
        else el.removeAttribute('style');
      });

      // Mobile min width with full logo: padding (20+20) + logo + 16px gap + button + 16px gap + hamburger
      const mobileMinFull = 40 + logoWidth + 16 + mobileBtnWidth + 16 + hamburgerWidth;
    ...
    <>
      ...
      <style jsx global>{`
        ...
        .mobile-nav-element.header-btn a {
          font-size: clamp(15px, calc(10px + 0.40vw), 22px) !important;
        ...
        @media (max-width: 81.25rem) {
          ...
          .logo-container:not(.logo-compact) {

...
/**
 * Forgot Username Modal Component
 * Opens as a popup to request username reminder email
 */
...
export default function ForgotUsernameModal({ isOpen, onClose }: ForgotUsernameModalProps) {
  ...
  const onSubmit = async (values: ForgotUsernameFormValues) => {
    ...
    try {
      const response = await fetch('/api/auth/username-recovery/request', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok && response.status !== 429) {
        throw new Error(data.message || 'Failed to send username reminder');
      }
      ...
      {success ? (
        <div className="space-y-6 py-4">
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-center text-[#dcdbd5]">
              If an account exists for {maskedEmail}, you will receive your username shortly.
            </p>
            <SecondaryButton
              href="#"
              onClick={(e) => {
        ...
        <div className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-[#e5e4dd]">Forgot Username?</h2>

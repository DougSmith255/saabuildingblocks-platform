/**
 * Add SAA Logo to Email Templates
 * Adds the SAA SVG logo to the header of all holiday email templates
 */
...
async function addLogoToTemplates() {
  try {
    console.log('=== FETCHING ALL ACTIVE HOLIDAY EMAIL TEMPLATES ===');
    ...
    for (const template of templates) {
      ...
      // Find the header section (div with text-align:center at the beginning)
      // Add logo right after the opening div tag of the header
      const headerMatch = updatedHtml.match(/<div style="text-align:center;padding:40px 20px 20px;background:linear-gradient/);

      if (!headerMatch) {
        console.log('  ⚠️  Could not find header section, skipping...\n');
        continue;
      }

      // Find the position right after the opening div tag
      const insertPosition = updatedHtml.indexOf('>', headerMatch.index!) + 1;

      // Insert the logo with proper spacing
      const logoHtml = `
        <!-- SAA Logo -->
      ...
      updatedHtml =
        updatedHtml.substring(0, insertPosition) +
        logoHtml +
        updatedHtml.substring(insertPosition);
    ...
    console.log('The SAA logo has been added to all template headers');
